...
import { 
  Loader2, QrCode, Upload, X, ArrowRight, Store, ShieldCheck, Clock
} from "lucide-react";
import { toast } from "sonner";
import { Database } from "@/integrations/supabase/types";

// Extracted from our modifications to suppliers table
type Supplier = any;

interface Props {
  supplier: Supplier | null;
  onSave: (updates: Partial<Supplier>) => Promise<any>;
}

export const SupplierSettings = ({ supplier, onSave }: Props) => {
  const [form, setForm] = useState({
    ...
    accepts_razorpay: supplier?.accepts_razorpay || false,
    use_custom_razorpay: supplier?.use_custom_razorpay || false,
    razorpay_key_id: supplier?.razorpay_key_id || "",
    razorpay_key_secret: supplier?.razorpay_key_secret || "",
    qr_code_url: supplier?.qr_code_url || "",
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Validate bank details if trying to be verified
      ...
      toast.success("Settings updated! Verification submitted.");
    } catch (error) {
  ...
  if (!supplier) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card rounded-xl border border-border p-10 text-center shadow-card">
        <p className="text-muted-foreground mb-4">No supplier record found.</p>
      </motion.div>
    );
  }
  ...
  const renderProfileForm = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex items-center justify-between">
        <h3 className="font-display font-semibold text-xl text-foreground">Manufacturer Profile</h3>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Tell partners about your manufacturing capabilities..."
        />
      </div>
      ...
      <div className="p-6 rounded-3xl bg-accent/5 border border-accent/10 space-y-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">Accept B2B payments via Razorpay.</p>
          </div>
          <div className="pt-1">
            <input 
              type="checkbox" 
              ...
              onClick={() => {
                ...
                input.onchange = async (e) => {
                  ...
                  try {
                    ...
                    const filePath = `supplier-qrs/${supplier?.id}/${Math.random()}.${fileExt}`;
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className={`p-8 rounded-[2.5rem] border-2 flex flex-col items-center text-center gap-6 transition-all ${
        ...
        <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center shadow-lg ${
          supplier?.verified ? "bg-blue-500 text-white shadow-blue-500/20" : "bg-amber-500 text-white shadow-amber-500/20"
        }`}>
          {supplier?.verified ? <ShieldCheck className="w-10 h-10" /> : <Clock className="w-10 h-10" />}
        </div>
        
        <div className="space-y-2">
          <h4 className="text-2xl font-bold text-foreground">
            {supplier?.verified ? "You are a Verified Manufacturer" : "Verification is Pending"}
          </h4>
          <p className="text-muted-foreground text-sm max-w-sm leading-relaxed">
            {supplier?.verified 
              ? "Your business is fully verified. Customers see the verification badge on your profile and products." 
            ...
            <div className="bg-white/50 p-4 rounded-xl border border-dashed border-border text-left">
              ...
              <ul className="space-y-1.5">
                <li className="flex items-center gap-2 text-xs">
                  {form.bank_name && form.bank_account_number ? <CheckCircle2 className="w-3 h-3 text-success" /> : <X className="w-3 h-3 text-destructive" />}
                  <span>Bank Account Details</span>
                </li>
                <li className="flex items-center gap-2 text-xs">
                  {form.upi_id ? <CheckCircle2 className="w-3 h-3 text-success" /> : <X className="w-3 h-3 text-destructive" />}
  ...
  const renderMenu = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in duration-500">
      {[
        { id: "profile", title: "Manufacturer Profile", icon: Store, desc: "Manage name, contact, and address", color: "text-coral bg-coral/10" },
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 max-w-4xl mx-auto pb-20">
      <div className="flex flex-col gap-1">
        <h2 className="text-3xl font-display font-bold text-foreground italic">Manufacturer Settings</h2>
        <p className="text-muted-foreground">Manage your identity, visibility, and money collection methods.</p>
      </div>
