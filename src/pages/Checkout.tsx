...
const Checkout = () => {
  ...
  const deliveryOptions = [
    ...
    { id: "home-delivery", label: "Home Delivery", description: "Standard home delivery", price: 59, time: "3-5 days delivery", icon: Truck },
  ];
  ...
  const paymentOptions = [
    ...
    { id: "manual", label: "Direct Payment", description: "UPI/Bank Transfer to Shop", icon: IndianRupee },
  ];
            ...
            {[
              ...
              <div key={s.id} className="flex items-center gap-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold text-sm ${
                  step === (s.id as any) ? "bg-accent text-accent-foreground" :
                ...
                <span className={`text-sm font-medium hidden sm:block ${
                  step === (s.id as any) ? "text-foreground" : "text-muted-foreground"
                }`}>{s.label}</span>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className={`text-xs mt-1 ${deliveryMethod === option.id ? "text-accent font-medium" : "text-muted-foreground"}`}>
                                  {option.time}
                                </p>
                              </div>
                            </div>
                        ...
                        {paymentOptions.map((option) => (
                          <button key={option.id} onClick={() => setPaymentMethod(option.id)}
                            className={`p-4 rounded-xl border-2 text-center transition-all ${
                              paymentMethod === option.id ? "border-accent bg-accent/5" : "border-border hover:border-accent/30"
                            }`}>
                            <option.icon className={`w-6 h-6 mx-auto mb-2 ${paymentMethod === option.id ? "text-accent" : "text-muted-foreground"}`} />
                      ...
                      {paymentMethod === "upi" && (
                        <div className="p-4 bg-success/5 rounded-lg border border-success/10 mb-6 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm text-foreground font-medium">Selected: UPI (GPay, PhonePe, Paytm)</span>
                          </div>
                          <Badge variant="outline" className="text-[10px] text-success border-success/30">Fastest</Badge>
                        </div>
                      )}
            ...
            <div className="lg:col-span-1">
              ...
              >
                ...
                {items.map((item) => {
                  const product = (item as CartItem).product;
                  return (
                    <div key={item.id} className="flex flex-col gap-4 mb-6 border-b border-border pb-6 last:border-0 last:mb-0 last:pb-0">
                      <div className="flex gap-3">
                        <div className="w-14 h-14 rounded-lg bg-secondary flex items-center justify-center shrink-0 overflow-hidden">
                          {item.design_file_url || (item.specifications as any)?.frontDesign ? (
                            <img src={item.design_file_url || (item.specifications as any)?.frontDesign} alt="Front Design" className="w-full h-full object-cover" />
                          ) : (
                        ...
                        <p className="font-semibold text-foreground text-sm">₹{((item.specifications as any)?.total || (product?.base_price || 0) * item.quantity).toLocaleString("en-IN")}</p>
                      </div>
                      
                      {(item.specifications as any)?.sides === "double" && (
                        <div className="flex items-center gap-3 pl-4 border-l-2 border-accent/20">
                          <div className="w-10 h-10 rounded bg-secondary flex items-center justify-center shrink-0 overflow-hidden">
                            {(item.specifications as any)?.backDesign ? (
                              <img src={(item.specifications as any).backDesign} alt="Back Design" className="w-full h-full object-cover" />
                            ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Reverse Side Design</p>
                            <p className="text-xs text-foreground truncate">
                              {(item.specifications as any)?.useSameImage ? "Same as front side" : "Custom back design"}
                            </p>
                          </div>
                        </div>
                      )}
